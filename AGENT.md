...
## 📸 Раздел 1.1. Работа с Медиа, Памятью и Оборудованием (MediaStream & OOM)

> [!CAUTION]
> **Утечки памяти в Safari (iOS OOM Crash)**
> Устройства Apple (и браузерный движок WebKit) крайне агрессивно закрывают вкладки (краш) при нецелевом расходовании памяти или зависании микрофона/камеры. Чтобы избежать белого экрана ("краша системы") при отправке аудио и фото:

1. **Запрет `FileReader` для камерных фотографий:**
   Никогда не использовать `FileReader.readAsDataURL()` для работы с фото с мобильной камеры. Оригинальное фото на 10-15 МБ при конвертации в Base64 ложится в JS-heap дважды, сразу "съедая" ~50-80 МБ ОЗУ, что убивает вкладку на iPhone. 
   **Правильный путь:** Всегда использовать модульный метод `compressFile()` из `utils/image.ts`, основанный на `URL.createObjectURL()`. Это потоковая экономичная загрузка без копирования Blob в память JS.

2. **Моментальное гашение `MediaStream` (Аудио/Микрофон):**
   Исторический баг — не оставлять активными треки микрофона (красная точка записи) во время сетевого запроса! После остановки диктофона (`MediaRecorder.onstop`) **всегда** моментально закрывать поток `stream.getTracks().forEach(t => t.stop())`. Если микрофон висит включенным, пока приложение ждет `sendVoiceToN8N` (Network Fetch, от 5 до 20 секунд), Safari расценивает это как мертвую прослушку и аварийно убивает/крашит страницу, и ответ ИИ никогда не доходит до пользователя.

3. **Особенности PWA, Telegram Web App (TMA) и Веба:**
   - **Строгий SSL/HTTPS**: Telegram Mini App, PWA и Camera API требуют исключительно безопасный контекст. Использование `http://` приведет к полному отказу TMA открыть приложение (белый/пустой экран).
   - **Скрытие нативного "Железа"**: Если платформа не нативная WEB/TMA (`Capacitor.isNativePlatform() === false`), из интерфейса должны быть вырезаны все функции синхронизации Apple Health / Google Health. Заголовок раздела настроек вместо "Здоровье и Активность" должен сокращаться до эстетичного "Питание".
   - **Скрытие Web-виджетов нативной среде**: На мобильных приложениях Capacitor (Android/iOS) категорически ЗАПРЕЩАЕТСЯ отображать встроенный виджет логина Telegram (`<script async src="https://telegram.org/js/telegram-widget.js">`). Он не способен пройти Origin-валидацию (т.к. приложение локально работает на `https://localhost`) и сломается с ошибкой 'Bot domain invalid'. В нативных режимах он скрыт — приоритетным нативным провайдером является Google (`GoogleAuth` SDK).
   - **TMA Layout Tweaks**: При инициализации в `Telegram` автоматически добавляется CSS-класс `body.is-tma`. С помощью этого класса можно вносить пиксельные корректировки (напр., отступ `SubHeader` или отступы клавиатуры) эксклюзивно для Telegram, не зацепляя Android/iOS.
...
## 📖 Раздел 1.4. Food Catalog API (Интеграция Справочника Продуктов)
В релизе `v3.22.19` локальный хардкод-каталог (`GLOBAL_CATALOG`) был полностью удалён из связки поиска, уступив место единому удаленному API справочнику.

### 1. Архитектура Поиска по Каталогу (`CatalogSearch` & `EditItemModal`)
- **Полный переход на Backend Searching**: Ввод пользователя отправляется на эндпоинт `/api/catalog/search?q={query}`.
- **Rules of Hooks**: Для реализации debounce (задержки 400мс) мы используем `useRef(setTimeout)`. **СВЯЩЕННОЕ ПРАВИЛО**: декларации хуков, например `useRef`, обязаны стоять ДО любых early returns (например: `if (!isOpen) return null;`), иначе React выбросит критическую ошибку (Error Boundary).
- **Динамические Подсказки**: При вводе названия продукта, компонент предлагает список продуктов из базы бэкенда вместе с заранее заполненными КБЖУ (макросами) на 100г.

### 2. Маппинг Категорий Бэкенда (Strict Dictionary Linking)
На стороне сервера существует **23 строгих английских slug-ключа** (например, `dairy`, `meat`, `ready_meals`).
Фронтенд **сохранил** свой оригинальный визуальный словарь (`25 русских категорий` и их эмодзи из `GROUP_ORDER`), чтобы не сломать исторические записи пользователей.
- Все ключи бэкенда связываются с фронтенд-группами строго через маппер (`mapBackendCategory` в `utils/data.ts`).
- Таким образом, при поступлении товара `category: "baby"` с бэкенда, фронт налету преобразовывает его во внутренний `Детское питание`, навешивая бейджики 🍼 и корректные цвета — не меняя CSS-классы и разметку.
...
> [!CAUTION]
> **ПРАВИЛО ИЗМЕНЕНИЯ ВЕРСИЙ (VERSION BUMP POLICY):**
> Любое логическое завершение функционала, исправление бага или выдача нового билда/результата пользователю **ОБЯЗАНО** сопровождаться поднятием версии в `package.json` (Bump Version). Промежуточные технические коммиты могут оставаться в текущей версии, но **любая презентация работы пользователю = новая версия**. Это железобетонное правило. Версии должны быть семантическими.
...
1. **Bump Version:** Обновить версию (`npm version patch/minor/major` или руками в `package.json`).
...
## 🛠 Раздел 1.5. Синхронизация и Data-Lock (Sync Guard)
Начиная с версии `v3.5.11`, фронтенд работает в режиме изолированной мутации `onSnapshot` от UI:
1. **Sync Guard Blocks**: Механизм `localMutationTime.current` защищает **только** локальные пользовательские мутации таблиц `list`, `stock`, `baseline`, `diary`.
2. **AI Messages Safe Insert**: Добавление сообщений ИИ или статусов `"Загрузка..."` больше не обновляет `localMutationTime`. Это позволяет фронтенду мгновенно получать обновления Firestore через `onSnapshot` сразу после завершения генерации ИИ, не дожидаясь истечения 10-секундного кулдауна.
3. **Безопасный Сброс (Factory Reset)**: Механизм сброса данных (`resetLocalState`) модифицирует массивы пользователя, однако оставляет нетронутыми `stats` (объект аналитики для проверки лимитов billing).

## 🧩 Раздел 5. Золотое Правило Аргументов
При вызовах между слоем визуализации (`App.tsx`, `ChatScreen.tsx`) и сетевым слоем (`ai.ts`, `api.ts`):
Строго запрещено передавать позиционные аргументы длиннее 3-х параметров в виде списка (`foo(a, b, c, d)`). Из-за этого уже возникал сбой перепутанных строк `userId, userEmail, tab`. Весь новый рефакторинг должен заворачивать опции в TypeScript Interface (`foo(params: Options Interface)`).
